...
<div class="max-w-3xl mx-auto py-10 px-4">
                ...
                <div class="w-full md:w-auto no-print">
                    @if($booking->status == 'pending')
                        <form action="{{ route('booking.pay', $booking->id) }}" method="POST">
                            @csrf
                            <button type="submit" class="w-full bg-orange-500 text-white px-10 py-4 rounded-2xl font-black uppercase hover:bg-orange-600 transition shadow-md">
                                Bayar Sekarang
                            </button>
                        </form>
                    @else
                        <button onclick="cetakTiket()" class="w-full bg-emerald-500 text-white px-10 py-4 rounded-2xl font-black uppercase hover:bg-emerald-600 transition shadow-md">
                            Cetak Tiket
                        </button>
                    @endif
                </div>
    ...
    <div class="mt-8 text-center no-print">
        <a href="{{ route('tickets.index') }}" class="text-gray-400 hover:text-gray-600 dark:hover:text-white font-bold text-sm uppercase tracking-widest transition flex items-center justify-center">
...
<script>
    function cetakTiket() {
        ...
        // Menggunakan onafterprint dengan fallback setTimeout agar pasti pindah halaman
        ...
        // Fallback jika onafterprint tidak terdeteksi oleh browser tertentu
        setTimeout(function() {
...
<style>
    @media print {
        nav, .no-print, .notif, button, a { display: none !important; }
        body { background: white !important; color: black !important; }
        .dark { background: white !important; }
        p, h2, span, label { color: black !important; }
